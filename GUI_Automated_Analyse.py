...
#%% Labels
class AutomatedAnalyse(QMainWindow):
    def __init__(self,parent=None,ImageName=None,GroundTruth=None,JunctionalLabel=None):
        super(AutomatedAnalyse,self).__init__(parent)
        self.ParentLink = parent
        self.ImageName = ImageName
        self.GroundTruth = GroundTruth
        self.JunctionalLabel = JunctionalLabel
        self.initUI()

    def initUI(self):
        self.QuickKill = False
        self.CentralWindow = AutomatedAnalyseWindow(self)
...
class AutomatedAnalyseWindow(QWidget):
    def __init__(self,parent=None):
        super(AutomatedAnalyseWindow,self).__init__(parent)
        self.ParentLink = parent
        self.Dilation = 1
        self.initUI()
    ...
    def Analyse(self):
        print(self.sldAnalisis.sliderPosition())
        #self.TrainingsImageName = self.ComboBoxTraining.currentText()
        #self.Labels = (int(self.LineEditLabel1.text()),int(self.LineEditLabel2.text()),int(self.LineEditLabel3.text()))
        #self.ParentLink.QuickKill = True
        #self.ParentLink.close()
    
    def LoadPatches(self):
        self.tmp_image = imread(self.ParentLink.ImageName)
        self.tmp_image = cv2.split(self.tmp_image)[self.ParentLink.JunctionalLabel]
